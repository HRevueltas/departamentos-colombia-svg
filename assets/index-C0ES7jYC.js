...
`,a.match(/^<?\w[^>]*[^\/]$/)&&!a.startsWith("?")&&(e+="  ")}),t.substring(1,t.length-3)}function g(d){const t=document.getElementById("loading");t&&(t.style.display=d?"block":"none")}function f(d){return d.replace(/[^\w-]/g,"\\$&")}function m(d,t="success"){const e=document.createElement("div");e.className=`notification ${t}`,e.textContent=d,e.style.cssText=`
  ...
  `,document.body.appendChild(e),setTimeout(()=>{e.style.animation="slideOut 0.3s ease",setTimeout(()=>e.remove(),300)},3e3)}function x(d){const t=new Blob([d]).size,e=t/1024;return e<1?`${t} bytes`:`${e.toFixed(2)} KB`}class A{svg;projection;path;zoom;g=null;constructor(t){this.svg=v(`#${t}`).append("svg").attr("id","mapa-svg").attr("viewBox",`0 0 ${r.mapWidth} ${r.mapHeight}`).attr("preserveAspectRatio","xMidYMid meet"),this.projection=b(),this.path=y().projection(this.projection),this.zoom=S().scaleExtent([.5,8]).on("zoom",e=>{this.g&&this.g.attr("transform",e.transform)}),this.svg.call(this.zoom)}renderMapa(t){if(this.svg.selectAll("*").remove(),n.currentFeatures=t,t.length===0)return;const e={type:"FeatureCollection",features:t};this.projection.fitSize([r.mapWidth,r.mapHeight],e),this.g=this.svg.append("g").attr("class","zoom-group"),this.g.append("g").attr("class","municipios").selectAll("path").data(t).enter().append("path").attr("class","municipio").attr("data-element-id",(a,s)=>a.properties.MPIO_CCNCT||`municipio-${s}`).attr("name",a=>a.properties.MPIO_CNMBR||"").attr("fill",r.colors.fill).attr("stroke",r.colors.stroke).attr("stroke-width",r.colors.strokeWidth).attr("d",this.path),this.svg.call(this.zoom.transform,h)}renderMunicipios(t){!n.allData.municipios||n.allData.municipios.features.filter(o=>o.properties.DPTO_CCDGO===t).length===0||this.svg.selectAll(".departamento").attr("stroke-width","0.3").attr("stroke-opacity","0.5")}renderEtiquetas(t){if(!this.g)return;this.g.append("g").attr("class","etiquetas").selectAll("text").data(t).enter().append("text").attr("class","etiqueta").attr("data-element-id",(o,a)=>o.properties.MPIO_CCNCT||`etiqueta-${a}`).attr("name",o=>o.properties.MPIO_CNMBR||"").attr("x",o=>this.path.centroid(o)[0]).attr("y",o=>this.path.centroid(o)[1]).attr("text-anchor","middle").attr("font-size","10px").attr("fill","#333").attr("font-weight","bold").text(o=>o.properties.MPIO_CNMBR||"")}renderPuntos(t){if(!this.g)return;this.g.append("g").attr("class","puntos").selectAll("circle").data(t).enter().append("circle").attr("class","punto").attr("data-element-id",(o,a)=>o.properties.MPIO_CCNCT||`punto-${a}`).attr("name",o=>o.properties.MPIO_CNMBR||"").attr("cx",o=>this.path.centroid(o)[0]).attr("cy",o=>this.path.centroid(o)[1]).attr("r",3).attr("fill","#d32f2f").attr("stroke",r.colors.stroke).attr("stroke-width","1")}updateDisplayOption(t){n.displayOption=t,this.svg.selectAll(".etiquetas").remove(),this.svg.selectAll(".puntos").remove(),t==="map+labels"?this.renderEtiquetas(n.currentFeatures):t==="map+points"&&this.renderPuntos(n.currentFeatures)}highlightElement(t){const e=f(t);this.svg.select(`[data-id="${e}"]`).attr("stroke","#ff9800").attr("stroke-width","2")}unhighlightElement(t){const e=f(t);this.svg.select(`[data-id="${e}"]`).attr("stroke",r.colors.stroke).attr("stroke-width",r.colors.strokeWidth)}getSVGElement(){return this.svg.node()}clearMap(){this.svg.selectAll("*").remove(),n.currentFeatures=[]}zoomIn(){this.svg.transition().duration(300).call(this.zoom.scaleBy,1.5)}zoomOut(){this.svg.transition().duration(300).call(this.zoom.scaleBy,.67)}resetZoom(){this.svg.transition().duration(300).call(this.zoom.transform,h)}}var z=E();const D=C(z);class L{projection;path;constructor(){this.projection=b(),this.path=y().projection(this.projection)}prepareSVGForExport(t,e,o){const a=t.cloneNode(!0);e||a.querySelectorAll(".etiquetas").forEach(c=>c.remove()),o?a.querySelectorAll(".punto").forEach(c=>{c.style.opacity="0"}):a.querySelectorAll(".puntos").forEach(c=>c.remove()),this.applyStylesToSVG(a);const s=a.getBBox(),i=`${s.x} ${s.y} ${s.width} ${s.height}`;return a.setAttribute("viewBox",i),a.setAttribute("xmlns","http://www.w3.org/2000/svg"),a.removeAttribute("width"),a.removeAttribute("height"),`<?xml version="1.0" encoding="UTF-8"?>
`+new XMLSerializer().serializeToString(a)}applyStylesToSVG(t){t.querySelectorAll(".departamento").forEach(e=>{e.style.fill=r.colors.fill,e.style.stroke=r.colors.stroke,e.style.strokeWidth=r.colors.strokeWidth}),t.querySelectorAll(".municipio").forEach(e=>{e.style.fill="none",e.style.stroke=r.colors.stroke,e.style.strokeWidth="0.3",e.style.opacity="0.5"}),t.querySelectorAll(".etiqueta").forEach(e=>{e.style.fontSize="10px",e.style.fill="#333",e.style.fontWeight="bold",e.style.textAnchor="middle"}),t.querySelectorAll(".punto").forEach(e=>{e.style.fill="#d32f2f",e.style.stroke=r.colors.stroke,e.style.strokeWidth="1"})}generateSVGForDepartment(t){if(!n.allData.departamentos)return"";const e=n.allData.departamentos.features.filter(i=>i.properties.DPTO_CCDGO===t);if(e.length===0)return"";const o=document.createElementNS("http://www.w3.org/2000/svg","svg");o.setAttribute("width",String(r.mapWidth)),o.setAttribute("height",String(r.mapHeight));const a={type:"FeatureCollection",features:e};this.projection.fitSize([r.mapWidth,r.mapHeight],a);const s=document.createElementNS("http://www.w3.org/2000/svg","g");return s.setAttribute("class","municipios"),e.forEach((i,l)=>{const p=document.createElementNS("http://www.w3.org/2000/svg","path");p.setAttribute("class","municipio");const c=i.properties.MPIO_CCNCT||`municipio-${l}`,u=i.properties.MPIO_CNMBR||"";p.setAttribute("data-element-id",c),p.setAttribute("name",u),p.setAttribute("d",this.path(i)||""),s.appendChild(p)}),o.appendChild(s),n.displayOption==="map+labels"?this.addEtiquetasToTempSVG(o,e):n.displayOption==="map+points"&&this.addPuntosToTempSVG(o,e),this.prepareSVGForExport(o,n.displayOption==="map+labels",n.displayOption==="map+points")}addEtiquetasToTempSVG(t,e){const o=document.createElementNS("http://www.w3.org/2000/svg","g");o.setAttribute("class","etiquetas"),e.forEach((a,s)=>{const i=this.path.centroid(a),l=document.createElementNS("http://www.w3.org/2000/svg","text");l.setAttribute("class","etiqueta");const p=a.properties.MPIO_CCNCT||`etiqueta-${s}`,c=a.properties.MPIO_CNMBR||"";l.setAttribute("data-element-id",p),l.setAttribute("name",c),l.setAttribute("x",String(i[0])),l.setAttribute("y",String(i[1])),l.textContent=c,o.appendChild(l)}),t.appendChild(o)}addPuntosToTempSVG(t,e){const o=document.createElementNS("http://www.w3.org/2000/svg","g");o.setAttribute("class","puntos"),e.forEach((a,s)=>{const i=this.path.centroid(a),l=document.createElementNS("http://www.w3.org/2000/svg","circle");l.setAttribute("class","punto");const p=a.properties.MPIO_CCNCT||`punto-${s}`,c=a.properties.MPIO_CNMBR||"";l.setAttribute("data-element-id",p),l.setAttribute("name",c),l.setAttribute("cx",String(i[0])),l.setAttribute("cy",String(i[1])),l.setAttribute("r","3"),o.appendChild(l)}),t.appendChild(o)}async downloadAllDepartments(){const t=new D;for(const s of r.departmentCodes){const i=this.generateSVGForDepartment(s.code);i&&t.file(`${s.name}.svg`,i)}const e=await t.generateAsync({type:"blob",compression:"DEFLATE",compressionOptions:{level:6}}),o=URL.createObjectURL(e),a=document.createElement("a");a.href=o,a.download="departamentos-colombia.zip",a.click(),URL.revokeObjectURL(o),m("ZIP descargado exitosamente")}downloadSingleDepartment(t,e){const o=this.generateSVGForDepartment(t);if(!o){m("Error al generar SVG","error");return}const a=new Blob([o],{type:"image/svg+xml"}),s=URL.createObjectURL(a),i=document.createElement("a");i.href=s,i.download=`${e}.svg`,i.click(),URL.revokeObjectURL(s),m("SVG descargado exitosamente")}}class I{codeDisplay;mapRenderer;constructor(t,e){const o=document.getElementById(t);if(!o)throw new Error(`Element with id ${t} not found`);this.codeDisplay=o,this.mapRenderer=e}updateSVGCode(){const t=this.mapRenderer.getSVGElement();if(!t)return;const e=t.cloneNode(!0);n.displayOption==="map"?e.querySelectorAll(".etiquetas, .puntos").forEach(s=>s.remove()):n.displayOption==="map+labels"?e.querySelectorAll(".puntos").forEach(s=>s.remove()):n.displayOption==="map+points"&&(e.querySelectorAll(".etiquetas").forEach(s=>s.remove()),e.querySelectorAll(".punto").forEach(s=>{s.style.opacity="0"}));const a=new XMLSerializer().serializeToString(e);this.renderCode(a),this.updateSizeInfo(a)}renderCode(t){const e=w(t);this.codeDisplay.innerHTML="",e.split(`
