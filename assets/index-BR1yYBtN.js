...
`,a.match(/^<?\w[^>]*[^\/]$/)&&!a.startsWith("?")&&(e+="  ")}),t.substring(1,t.length-3)}function g(d){const t=document.getElementById("loading");t&&(t.style.display=d?"block":"none")}function f(d){return d.replace(/[^\w-]/g,"\\$&")}function m(d,t="success"){const e=document.createElement("div");e.className=`notification ${t}`,e.textContent=d,e.style.cssText=`
  ...
  `,document.body.appendChild(e),setTimeout(()=>{e.style.animation="slideOut 0.3s ease",setTimeout(()=>e.remove(),300)},3e3)}function x(d){const t=new Blob([d]).size,e=t/1024;return e<1?`${t} bytes`:`${e.toFixed(2)} KB`}class A{svg;projection;path;zoom;g=null;constructor(t){this.svg=v(`#${t}`).append("svg").attr("id","mapa-svg").attr("viewBox",`0 0 ${n.mapWidth} ${n.mapHeight}`).attr("preserveAspectRatio","xMidYMid meet"),this.projection=b(),this.path=y().projection(this.projection),this.zoom=S().scaleExtent([.5,8]).on("zoom",e=>{this.g&&this.g.attr("transform",e.transform)}),this.svg.call(this.zoom)}renderMapa(t){if(this.svg.selectAll("*").remove(),r.currentFeatures=t,t.length===0)return;const e={type:"FeatureCollection",features:t};this.projection.fitSize([n.mapWidth,n.mapHeight],e),this.g=this.svg.append("g").attr("class","zoom-group"),this.g.append("g").attr("class","municipios").selectAll("path").data(t).enter().append("path").attr("class","municipio").attr("data-element-id",(a,s)=>a.properties.MPIO_CCNCT||`municipio-${s}`).attr("name",a=>a.properties.MPIO_CNMBR||"").attr("fill",n.colors.fill).attr("stroke",n.colors.stroke).attr("stroke-width",n.colors.strokeWidth).attr("d",this.path),this.svg.call(this.zoom.transform,h)}renderMunicipios(t){!r.allData.municipios||r.allData.municipios.features.filter(o=>o.properties.DPTO_CCDGO===t).length===0||this.svg.selectAll(".departamento").attr("stroke-width","0.3").attr("stroke-opacity","0.5")}renderEtiquetas(t){if(!this.g)return;this.g.append("g").attr("class","etiquetas").selectAll("text").data(t).enter().append("text").attr("class","etiqueta").attr("data-element-id",(o,a)=>o.properties.MPIO_CCNCT||`etiqueta-${a}`).attr("name",o=>o.properties.MPIO_CNMBR||"").attr("x",o=>this.path.centroid(o)[0]).attr("y",o=>this.path.centroid(o)[1]).attr("text-anchor","middle").attr("font-size","10px").attr("fill","#333").attr("font-weight","bold").text(o=>o.properties.MPIO_CNMBR||"")}renderPuntos(t){if(!this.g)return;this.g.append("g").attr("class","puntos").selectAll("circle").data(t).enter().append("circle").attr("class","punto").attr("data-element-id",(o,a)=>o.properties.MPIO_CCNCT||`punto-${a}`).attr("name",o=>o.properties.MPIO_CNMBR||"").attr("cx",o=>this.path.centroid(o)[0]).attr("cy",o=>this.path.centroid(o)[1]).attr("r",3).attr("fill","#d32f2f").attr("stroke",n.colors.stroke).attr("stroke-width","1")}updateDisplayOption(t){r.displayOption=t,this.svg.selectAll(".etiquetas").remove(),this.svg.selectAll(".puntos").remove(),t==="map+labels"?this.renderEtiquetas(r.currentFeatures):t==="map+points"&&this.renderPuntos(r.currentFeatures)}highlightElement(t){const e=f(t);this.svg.select(`[data-id="${e}"]`).attr("stroke","#ff9800").attr("stroke-width","2")}unhighlightElement(t){const e=f(t);this.svg.select(`[data-id="${e}"]`).attr("stroke",n.colors.stroke).attr("stroke-width",n.colors.strokeWidth)}getSVGElement(){return this.svg.node()}clearMap(){this.svg.selectAll("*").remove(),r.currentFeatures=[]}zoomIn(){this.svg.transition().duration(300).call(this.zoom.scaleBy,1.5)}zoomOut(){this.svg.transition().duration(300).call(this.zoom.scaleBy,.67)}resetZoom(){this.svg.transition().duration(300).call(this.zoom.transform,h)}}var D=E();const z=C(D);class I{projection;path;constructor(){this.projection=b(),this.path=y().projection(this.projection)}prepareSVGForExport(t,e,o){const a=t.cloneNode(!0);e||a.querySelectorAll(".etiquetas").forEach(c=>c.remove()),o?a.querySelectorAll(".punto").forEach(c=>{c.style.opacity="0"}):a.querySelectorAll(".puntos").forEach(c=>c.remove()),this.applyStylesToSVG(a);const s=a.getBBox(),i=`${s.x} ${s.y} ${s.width} ${s.height}`;return a.setAttribute("viewBox",i),a.setAttribute("xmlns","http://www.w3.org/2000/svg"),a.removeAttribute("width"),a.removeAttribute("height"),`<?xml version="1.0" encoding="UTF-8"?>
`+new XMLSerializer().serializeToString(a)}applyStylesToSVG(t){t.querySelectorAll(".departamento").forEach(e=>{e.style.fill=n.colors.fill,e.style.stroke=n.colors.stroke,e.style.strokeWidth=n.colors.strokeWidth}),t.querySelectorAll(".municipio").forEach(e=>{e.style.fill="none",e.style.stroke=n.colors.stroke,e.style.strokeWidth="0.3",e.style.opacity="0.5"}),t.querySelectorAll(".etiqueta").forEach(e=>{e.style.fontSize="10px",e.style.fill="#333",e.style.fontWeight="bold",e.style.textAnchor="middle"}),t.querySelectorAll(".punto").forEach(e=>{e.style.fill="#d32f2f",e.style.stroke=n.colors.stroke,e.style.strokeWidth="1"})}generateSVGForDepartment(t){if(!r.allData.departamentos)return"";const e=r.allData.departamentos.features.filter(i=>i.properties.DPTO_CCDGO===t);if(e.length===0)return"";const o=document.createElementNS("http://www.w3.org/2000/svg","svg");o.setAttribute("width",String(n.mapWidth)),o.setAttribute("height",String(n.mapHeight));const a={type:"FeatureCollection",features:e};this.projection.fitSize([n.mapWidth,n.mapHeight],a);const s=document.createElementNS("http://www.w3.org/2000/svg","g");return s.setAttribute("class","municipios"),e.forEach((i,l)=>{const p=document.createElementNS("http://www.w3.org/2000/svg","path");p.setAttribute("class","municipio");const c=i.properties.MPIO_CCNCT||`municipio-${l}`,u=i.properties.MPIO_CNMBR||"";p.setAttribute("data-element-id",c),p.setAttribute("name",u),p.setAttribute("d",this.path(i)||""),s.appendChild(p)}),o.appendChild(s),r.displayOption==="map+labels"?this.addEtiquetasToTempSVG(o,e):r.displayOption==="map+points"&&this.addPuntosToTempSVG(o,e),this.prepareSVGForExport(o,r.displayOption==="map+labels",r.displayOption==="map+points")}addEtiquetasToTempSVG(t,e){const o=document.createElementNS("http://www.w3.org/2000/svg","g");o.setAttribute("class","etiquetas"),e.forEach((a,s)=>{const i=this.path.centroid(a),l=document.createElementNS("http://www.w3.org/2000/svg","text");l.setAttribute("class","etiqueta");const p=a.properties.MPIO_CCNCT||`etiqueta-${s}`,c=a.properties.MPIO_CNMBR||"";l.setAttribute("data-element-id",p),l.setAttribute("name",c),l.setAttribute("x",String(i[0])),l.setAttribute("y",String(i[1])),l.textContent=c,o.appendChild(l)}),t.appendChild(o)}addPuntosToTempSVG(t,e){const o=document.createElementNS("http://www.w3.org/2000/svg","g");o.setAttribute("class","puntos"),e.forEach((a,s)=>{const i=this.path.centroid(a),l=document.createElementNS("http://www.w3.org/2000/svg","circle");l.setAttribute("class","punto");const p=a.properties.MPIO_CCNCT||`punto-${s}`,c=a.properties.MPIO_CNMBR||"";l.setAttribute("data-element-id",p),l.setAttribute("name",c),l.setAttribute("cx",String(i[0])),l.setAttribute("cy",String(i[1])),l.setAttribute("r","3"),o.appendChild(l)}),t.appendChild(o)}async downloadAllDepartments(){const t=new z;for(const s of n.departmentCodes){const i=this.generateSVGForDepartment(s.code);i&&t.file(`${s.name}.svg`,i)}const e=await t.generateAsync({type:"blob",compression:"DEFLATE",compressionOptions:{level:6}}),o=URL.createObjectURL(e),a=document.createElement("a");a.href=o,a.download="departamentos-colombia.zip",a.click(),URL.revokeObjectURL(o),m("ZIP descargado exitosamente")}downloadSingleDepartment(t,e){const o=this.generateSVGForDepartment(t);if(!o){m("Error al generar SVG","error");return}const a=new Blob([o],{type:"image/svg+xml"}),s=URL.createObjectURL(a),i=document.createElement("a");i.href=s,i.download=`${e}.svg`,i.click(),URL.revokeObjectURL(s),m("SVG descargado exitosamente")}}class L{codeDisplay;mapRenderer;constructor(t,e){const o=document.getElementById(t);if(!o)throw new Error(`Element with id ${t} not found`);this.codeDisplay=o,this.mapRenderer=e}updateSVGCode(){const t=this.mapRenderer.getSVGElement();if(!t)return;const e=t.cloneNode(!0);r.displayOption==="map"?e.querySelectorAll(".etiquetas, .puntos").forEach(s=>s.remove()):r.displayOption==="map+labels"?e.querySelectorAll(".puntos").forEach(s=>s.remove()):r.displayOption==="map+points"&&(e.querySelectorAll(".etiquetas").forEach(s=>s.remove()),e.querySelectorAll(".punto").forEach(s=>{s.style.opacity="0"}));const a=new XMLSerializer().serializeToString(e);this.renderCode(a),this.updateSizeInfo(a)}renderCode(t){const e=w(t);this.codeDisplay.innerHTML="",e.split(`
`).forEach((a,s)=>{const i=document.createElement("div");i.className="code-line";const l=document.createElement("span");l.className="line-number",l.textContent=String(s+1);const p=document.createElement("span");p.className="line-content",p.textContent=a;const c=a.match(/class="(municipio|etiqueta|punto)"/),u=a.match(/data-element-id="([^"]+)"/);c&&u&&(i.classList.add("code-hoverable"),i.dataset.elementType=c[1],i.dataset.elementId=u[1]),i.appendChild(l),i.appendChild(p),this.codeDisplay.appendChild(i)}),this.setupCodeHoverEvents()}setupCodeHoverEvents(){this.codeDisplay.querySelectorAll(".code-hoverable").forEach(e=>{e.addEventListener("mouseenter",o=>{const s=o.currentTarget.dataset.elementId;s&&this.highlightSVGElement(s)}),e.addEventListener("mouseleave",o=>{const s=o.currentTarget.dataset.elementId;s&&this.unhighlightSVGElement(s)})})}highlightSVGElement(t){const o=this.mapRenderer.getSVGElement().querySelector(`[data-element-id="${t}"]`);if(o){const a=o;o.tagName==="text"?(a.style.fill="#ff9800",a.style.fontWeight="bold",a.style.fontSize="12px"):(a.style.stroke="#ff9800",a.style.strokeWidth="2",o.tagName==="path"&&(a.style.fill="#ffa726"),a.style.opacity="1")}}unhighlightSVGElement(t){const o=this.mapRenderer.getSVGElement().querySelector(`[data-element-id="${t}"]`);if(o){const a=o,s=o.tagName==="text",i=a.classList.contains("municipio"),l=a.classList.contains("punto");s?(a.style.fill="#333",a.style.fontWeight="bold",a.style.fontSize="10px"):(a.style.stroke="#ffffff",a.style.strokeWidth=i?"0.5":"1",i?a.style.fill="#6f9c76":l&&(a.style.fill="#d32f2f"),a.style.opacity="")}}copyCodeToClipboard(){const t=Array.from(this.codeDisplay.querySelectorAll(".line-content")).map(e=>e.textContent).join(`
`);navigator.clipboard.writeText(t).then(()=>{const e=document.querySelector(".copy-code-btn");e&&(e.innerHTML='<i class="fa-solid fa-check"></i> Copiado',setTimeout(()=>{e.innerHTML='<i class="fa-solid fa-copy"></i> Copiar'},2e3))})}updateSizeInfo(t){const e=document.querySelector(".svg-size-info");if(e){const o=x(t);e.innerHTML=`<span class="size-value">${o}</span>`}}clearCode(){this.codeDisplay.innerHTML='<div class="code-placeholder"><i class="fa-solid fa-code"></i><p>Selecciona un departamento para ver el código SVG</p></div>';const t=document.querySelector(".svg-size-info");t&&(t.innerHTML=`
        <span class="size-placeholder">
          <i class="fa-solid fa-circle-info"></i>
          Sin selección
        </span>
      `)}}class G{mapRenderer;svgExporter;codeViewer;async init(){this.setupUI(),await this.loadData(),this.setupEventListeners(),this.loadFromURL()}setupUI(){document.querySelector("#app").innerHTML=`
      <div class="container">
        <header role="banner">
                ...
                <div class="control-group">
                  ...
                  <div class="select-wrapper">
                    ...
                    >
                      ...
                      ${n.departmentCodes.map(t=>`<option value="${t.code}">${t.name}</option>`).join("")}
                    </select>
    ...
    `,this.mapRenderer=new A("mapa-container"),this.svgExporter=new I,this.codeViewer=new L("code-display",this.mapRenderer)}async loadData(){g(!0);try{const t=await fetch(n.dataUrl).then(e=>e.json());r.allData.departamentos=t,r.allData.municipios=t,m("Datos cargados exitosamente")}catch(t){console.error("Error loading data:",t),m("Error al cargar los datos","error")}finally{g(!1)}}setupEventListeners(){const t=document.getElementById("departamento");t.addEventListener("change",()=>{this.handleDepartmentChange(t.value),this.updateURL(t.value)}),window.addEventListener("hashchange",()=>this.loadFromURL()),document.querySelectorAll('input[name="displayOption"]').forEach(e=>{e.addEventListener("change",o=>{const a=o.target.value;this.mapRenderer.updateDisplayOption(a),this.codeViewer.updateSVGCode()})}),document.getElementById("btn-descargar")?.addEventListener("click",()=>{const e=t.value,o=n.departmentCodes.find(a=>a.code===e)?.name||"";this.svgExporter.downloadSingleDepartment(e,o)}),document.getElementById("btn-copiar")?.addEventListener("click",()=>{const e=this.mapRenderer.getSVGElement();if(e){const o=this.svgExporter.prepareSVGForExport(e,r.displayOption==="map+labels",r.displayOption==="map+points");navigator.clipboard.writeText(o).then(()=>{m("SVG copiado al portapapeles")})}}),document.getElementById("btn-descargar-todos")?.addEventListener("click",()=>{this.svgExporter.downloadAllDepartments()}),document.querySelector(".copy-code-btn")?.addEventListener("click",()=>{this.codeViewer.copyCodeToClipboard()}),document.getElementById("btn-descargar-code")?.addEventListener("click",()=>{const o=document.getElementById("departamento").value,a=n.departmentCodes.find(s=>s.code===o)?.name||"";this.svgExporter.downloadSingleDepartment(o,a)}),document.getElementById("zoom-in")?.addEventListener("click",()=>{this.mapRenderer.zoomIn()}),document.getElementById("zoom-out")?.addEventListener("click",()=>{this.mapRenderer.zoomOut()}),document.getElementById("zoom-reset")?.addEventListener("click",()=>{this.mapRenderer.resetZoom()})}handleDepartmentChange(t){const e=document.querySelector("#mapa-container .map-placeholder");if(!t){this.disableButtons(),this.codeViewer.clearCode(),this.mapRenderer.clearMap(),e&&(e.style.display="flex",e.innerHTML=`
          <i class="fa-solid fa-map"></i>
          <p>Selecciona un departamento para visualizar el mapa</p>
        ...
        `);return}if(e&&(e.style.display="none"),r.isAllDepartments=!1,this.hideDownloadAllButton(),!r.allData.departamentos)return;const o=r.allData.departamentos.features.filter(a=>a.properties.DPTO_CCDGO===t);this.mapRenderer.renderMapa(o),this.mapRenderer.renderMunicipios(t),r.displayOption==="map+labels"?this.mapRenderer.renderEtiquetas(o):r.displayOption==="map+points"&&this.mapRenderer.renderPuntos(o),this.codeViewer.updateSVGCode(),this.enableButtons()}enableButtons(){document.getElementById("btn-descargar").disabled=!1,document.getElementById("btn-copiar").disabled=!1,document.getElementById("btn-descargar-code").disabled=!1}disableButtons(){document.getElementById("btn-descargar").disabled=!0,document.getElementById("btn-copiar").disabled=!0,document.getElementById("btn-descargar-code").disabled=!0}showDownloadAllButton(){document.getElementById("btn-descargar").style.display="none",document.getElementById("btn-copiar").style.display="none",document.getElementById("btn-descargar-todos").style.display="inline-block"}hideDownloadAllButton(){document.getElementById("btn-descargar").style.display="inline-block",document.getElementById("btn-copiar").style.display="inline-block",document.getElementById("btn-descargar-todos").style.display="none"}updateURL(t){if(!t||t==="ALL"){window.location.hash&&history.pushState(null,"",window.location.pathname);return}const e=n.departmentCodes.find(o=>o.code===t);if(e){const o=this.createSlug(e.name);history.pushState(null,"",`#${o}`)}}loadFromURL(){const t=window.location.hash.slice(1);if(!t)return;const e=n.departmentCodes.find(o=>this.createSlug(o.name)===t);if(e){const o=document.getElementById("departamento");o.value=e.code,this.handleDepartmentChange(e.code)}}createSlug(t){return t.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/\s+/g,"-").replace(/[^a-z0-9-]/g,"").replace(/-+/g,"-")}}const V=new G;V.init();
//# sourceMappingURL=index-BR1yYBtN.js.map
